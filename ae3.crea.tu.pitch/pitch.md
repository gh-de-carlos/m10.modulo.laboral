# Mi Elevator Pitch Profesional

![Elevator Pitch](./elevator.pitch.ia.jpg)

## Parte 1: Respuestas a las 5 Preguntas Clave

### 1. ¿Quién eres?
Soy Carlos Pizarro Morales, desarrollador Full Stack con certificación Talento Digital Chile, especializado en JavaScript, Vue.js, Java y Spring Boot.

### 2. ¿Qué haces bien?
Me destaco en desarrollo frontend con Vue.js y React, backend con Spring Boot y Node.js, y en refactorizar código para mejorar mantenibilidad y rendimiento. También tengo excelentes habilidades de comunicación y enseñanza, habiendo trabajado como tutor de bootcamp.

### 3. ¿Qué lograste o aprendiste?
Colaboré en el desarrollo de una aplicación web para Tecprosalud que lleva servicios médicos a zonas remotas de Chile. Refactoricé código base, implementé nuevas características con Vue.js y Spring Boot, y trabajé con PostgreSQL en un proyecto que genera impacto social real.

### 4. ¿Qué buscas?
Busco un rol como Desarrollador Full Stack Junior o Semi-Senior en un equipo colaborativo que valore el aprendizaje continuo, las buenas prácticas de desarrollo y donde pueda aportar mi experiencia técnica mientras sigo creciendo profesionalmente.

### 5. ¿Qué te diferencia?
Mi trayectoria diversa me da habilidades únicas: capacidad de adaptación a nuevos entornos, facilidad para comunicar conceptos técnicos complejos, y una perspectiva optimista que me permite enfrentar desafíos con creatividad. Además, mi experiencia enseñando en bootcamps me ha dado empatía y capacidad de trabajo en equipo.

---

## Parte 2: Mi Pitch 1.0 (Versión Inicial)

Soy Carlos Pizarro, desarrollador Full Stack certificado por Talento Digital Chile. Me especializo en JavaScript, Vue.js, React, Java y Spring Boot, y disfruto crear aplicaciones que generen impacto real. Colaboré en el desarrollo de una aplicación para Tecprosalud que lleva servicios médicos a zonas remotas, donde refactoricé código y desarrollé nuevas funcionalidades tanto en frontend como backend. Mi trayectoria diversa, pasando por retail, banca y educación, me ha dado habilidades de comunicación, adaptabilidad y trabajo en equipo que complementan mi perfil técnico. Busco un entorno donde pueda seguir creciendo profesionalmente mientras aporto soluciones escalables y código de calidad.

**Palabras:** 98

---

## Parte 3: Mi Pitch Mejorado (Versión Final)

Hola, soy Carlos Pizarro, desarrollador Full Stack con tres certificaciones de Talento Digital Chile. Me especializo en JavaScript, trabajando tanto en frontend con Vue.js y React, como en backend con Spring Boot y Node.js.

Recientemente colaboré en una aplicación para Tecprosalud que acerca servicios médicos a zonas remotas de Chile. Mi rol incluyó refactorizar código existente para mejorar su mantenibilidad e implementar nuevas características, trabajando con Vue.js, Spring Boot y PostgreSQL. Este proyecto me permitió aplicar buenas prácticas de desarrollo mientras contribuía a un producto con impacto social.

Lo que me diferencia es mi trayectoria única: antes de reconvertirme al desarrollo, trabajé en retail, banca y como tutor de bootcamp. Esto me dio habilidades que complementan mi perfil técnico: excelente comunicación, capacidad de adaptarme rápidamente a nuevos contextos, y facilidad para trabajar en equipo.

Busco unirme a un equipo que valore el código limpio, el aprendizaje continuo y la colaboración, donde pueda seguir creciendo como desarrollador mientras aporto mi experiencia y mi enfoque orientado a resolver problemas reales con tecnología.

**Tiempo estimado:** 55-60 segundos

---

## Parte 5: Reflexión Personal

### ¿Qué cambió entre mi versión inicial y la final?

Entre la versión inicial y la final, el pitch pasó de ser una enumeración de habilidades y experiencias a una narrativa más fluida y conversacional. Estructuré mejor el contenido: comencé con mi especialización técnica, seguí con un ejemplo concreto del proyecto Tecprosalud que incluye impacto medible, luego destaqué mi diferenciador (trayectoria diversa que aporta soft skills), y cerré con mi objetivo profesional claro. También eliminé tecnicismos innecesarios y me enfoqué en transmitir cómo mis experiencias previas complementan mi perfil técnico, no solo como un dato sino como un valor agregado.

### ¿Qué aprendí de la práctica con IA o del feedback?

Aprendí que un pitch efectivo no es solo listar logros, sino contar una historia coherente que conecte quién soy, qué hago bien, y qué busco. La IA me ayudó a identificar que mi primera versión era demasiado genérica en algunos puntos. El feedback me hizo consciente de la importancia de mencionar impacto concreto (proyecto Tecprosalud con contexto social) y de posicionar mi trayectoria diversa como una fortaleza, no como una desviación. También entendí que el pitch debe sonar natural al hablarlo, no como un texto leído, por lo que simplifiqué frases y usé un tono más cercano.

### ¿Cómo voy a usar mi pitch?

Voy a usar mi pitch en tres contextos principales: **primero**, en entrevistas presenciales o virtuales, adaptándolo según la empresa (por ejemplo, si buscan alguien con experiencia en Vue.js, enfatizo Tecprosalud; si valoran soft skills, destaco mi experiencia como tutor). **Segundo**, lo convertiré en un video corto para LinkedIn que acompañe mi perfil, mostrando mi capacidad de comunicación y mi story profesional de forma visual. **Tercero**, lo usaré en eventos de networking o meetups tech en Chile, donde puedo conectar con recruiters o developers de empresas como Mercado Libre, Bci o NotCo, que están en mi mapa de oportunidades. El pitch será mi carta de presentación que transmite confianza, claridad y propósito profesional.

---

## Versión Pitch para LinkedIn (Texto)

👨‍💻 Desarrollador Full Stack | Vue.js, React, Spring Boot | Talento Digital Chile

Hola, soy Carlos Pizarro. Después de una trayectoria en retail, banca y educación, encontré mi pasión en el desarrollo de software.

Hoy me especializo en JavaScript (Vue.js, React) y Java (Spring Boot), con tres certificaciones de Talento Digital Chile. Recientemente colaboré en una app que lleva servicios médicos a zonas remotas de Chile, refactorizando código y desarrollando nuevas funcionalidades.

Mi trayectoria diversa me dio algo valioso: capacidad de comunicar, adaptarme rápido y trabajar en equipo. Como ex-tutor de bootcamp, sé explicar conceptos técnicos con claridad.

🔍 Busco: Rol Full Stack Junior/Semi-Senior en equipo colaborativo que valore código limpio y aprendizaje continuo.

📍 Santiago, Chile | 🌐 Abierto a remoto/híbrido

#DesarrolladorFullStack #JavaScript #VueJS #SpringBoot #TalentoDigital #OpenToWork

---

## Notas sobre Uso del Pitch

### Adaptaciones según contexto:

**Para empresa financiera (ej. Bci):**
- Enfatizar experiencia en banca (Scotiabank) y comprensión de procesos críticos
- Destacar Spring Boot y PostgreSQL que son su stack principal
- Mencionar atención al detalle y responsabilidad

**Para startup (ej. NotCo):**
- Enfatizar Vue.js y capacidad de moverme rápido
- Destacar el impacto social del proyecto Tecprosalud (misión compartida)
- Mencionar adaptabilidad y ownership

**Para empresa grande (ej. Mercado Libre):**
- Destacar experiencia con arquitectura escalable
- Mencionar que fui tutor (mentalidad de mentoring que valoran)
- Enfatizar trabajo en equipo ágil

### Elementos clave a mantener siempre:
1. Stack técnico claro: Vue.js, React, Spring Boot, JavaScript
2. Proyecto Tecprosalud con impacto social
3. Certificaciones Talento Digital (3)
4. Trayectoria diversa como diferenciador positivo
5. Objetivo claro: Full Stack Junior/Semi-Senior

### Tips para la entrega oral:
- Sonreír naturalmente al hablar
- Mantener contacto visual (presencial) o mirar a la cámara (virtual)
- Usar pausas estratégicas después de mencionar Tecprosalud
- Énfasis en "impacto social" y "equipo colaborativo"
- Tono conversacional, no robótico
- Energía positiva que refleje mi optimismo
